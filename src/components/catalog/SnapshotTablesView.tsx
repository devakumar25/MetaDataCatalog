...
const mockTables: TableData[] = Array.from({ length: 20 }, (_, i) => ({
  ...
  tags: [
    ...(Math.random() > 0.7 ? ['snapshot-failed'] : []),
...
export const SnapshotTablesView: React.FC = () => {
  ...
  const [columns, setColumns] = useState<Column[]>([
    ...
    { key: 'dataSource', label: 'Source Report', visible: true, sortable: true },
    { key: 'schedule', label: 'Schedule Time', visible: true, sortable: true },
    { key: 'rowCount', label: 'Row Count', visible: true, sortable: true },
    { key: 'mainEngineSync', label: 'Last Ran', visible: true, sortable: true },
    { key: 'readEngine1Sync', label: 'Read Engine 1', visible: true, sortable: true },
    { key: 'readEngine2Sync', label: 'Read Engine 2', visible: true, sortable: true },
    { key: 'downstreamAssetsCount', label: 'Downstream Assets', visible: true, sortable: true },
    { key: 'actions', label: 'Actions', visible: true, sortable: false }
  ]);

  const tagCounts = {
    'snapshot-failed': mockTables.filter(t => t.tags.includes('snapshot-failed')).length,
    ...
    <div className="p-4">
      ...
      <div className="mb-6 grid grid-cols-2 gap-4">
          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200">
            ...
            <div className="text-2xl font-semibold">37,42,536 </div>
          </div>
        </div>
                  ...
                  {columns.filter(col => col.visible).map((column) => {
                    switch (column.key) {
                                  ...
                                  {table.tags.map(tag => (
                                    <span
                                      ...
                                      className={`
                                        ...
                                        ${tag === 'snapshot-failed' ? 'bg-red-100 text-red-800' : ''}
                      ...
                      case 'schedule':
                        return (
                          <td key={column.key} className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-500">{table.schedule}</div>
                          </td>
                        );  
                      case 'rowCount':
                        return (
